...
from yolox.losses import MultiBinLoss
...
class YOLOXHead3dMultiBin(YOLOXHead3d):
    def __init__(
        self,
        num_classes: int,
        width: float = 1.0,
        ...
        # ------------- loss function ---------------- #
        self.multibin_loss = MultiBinLoss(num_bin=self.num_bin, overlap=self.overlap, loss_weight=self.diff_weight)
    ...
    def forward(
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            x: torch.Tensor = self.stems[k](x)
            cls_x = x
            reg_x = x
            # --- 3d prediction --- #
            ddd_x = x

            cls_feat: torch.Tensor = cls_conv(cls_x)
            cls_output: torch.Tensor = self.td_cls_preds[k](cls_feat)

            reg_feat: torch.Tensor = reg_conv(reg_x)
            reg_output: torch.Tensor = self.td_reg_preds[k](reg_feat)
            obj_output: torch.Tensor = self.td_obj_preds[k](reg_feat)
    ...
    def get_losses(
        ...
        if self.lr_ph == "3d":
            ...
            loss_yaw, loss_yaw_conf = self.multibin_loss(
                yaw_preds.view(-1, 2)[fg_masks], yaw_conf_preds.view(-1, self.num_bin)[fg_masks], yaw_targets
            )
